...
<head>
    ...
    <title>Register - {{ __('messages.app_name') }}</title>
            ...
            <form action="{{ route('register') }}" method="POST" class="space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative flex items-center bg-white border border-slate-200 rounded-none px-4 py-3.5 focus-within:border-red-500 focus-within:ring-1 focus-within:ring-red-500 transition-all">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="{{ __('messages.register_email_placeholder') }}" class="w-full bg-transparent focus:outline-none text-slate-800 placeholder-slate-400 text-sm ml-3" required>
                    </div>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative flex items-center bg-white border border-slate-200 rounded-none px-4 py-3.5 focus-within:border-red-500 focus-within:ring-1 focus-within:ring-red-500 transition-all">
                        ...
                        <input type="password" name="password" id="password" placeholder="{{ __('messages.register_password_placeholder') }}" class="w-full bg-transparent focus:outline-none text-slate-800 placeholder-slate-400 text-sm ml-3 pr-10" required>
                        <!-- Show/Hide Toggle Button -->
                        <button type="button" onclick="togglePasswordVisibility('password', 'eye-icon-pw')" class="absolute right-4 text-slate-400 hover:text-slate-600 focus:outline-none">
                            <svg id="eye-icon-pw" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative flex items-center bg-white border border-slate-200 rounded-none px-4 py-3.5 focus-within:border-red-500 focus-within:ring-1 focus-within:ring-red-500 transition-all">
                        ...
                        <input type="password" name="password_confirmation" id="password_confirmation" placeholder="{{ __('messages.confirm_password_placeholder') }}" class="w-full bg-transparent focus:outline-none text-slate-800 placeholder-slate-400 text-sm ml-3 pr-10" required>
                        <!-- Show/Hide Toggle Button -->
                        <button type="button" onclick="togglePasswordVisibility('password_confirmation', 'eye-icon-cpw')" class="absolute right-4 text-slate-400 hover:text-slate-600 focus:outline-none">
                            <svg id="eye-icon-cpw" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                        </button>
                ...
                <!-- Sharp Submit Button -->
                <button type="submit" class="w-full py-4 bg-[#d5001c] hover:bg-[#b80010] text-white font-bold rounded-none transition-all shadow-lg shadow-red-600/10 text-sm mt-2">
                    {{ __('messages.create_account_btn') }}
                </button>
            </form>

            <!-- Footer Login link & Copyright inside Container -->
            <div class="pt-4 border-t border-slate-100 space-y-3">
                <div class="text-center text-sm text-slate-500 font-medium">
                    {{ __('messages.already_have_account') }}
                    <a href="{{ route('login') }}" class="text-[#d5001c] hover:underline ml-1 font-bold">{{ __('messages.login') }}</a>
                </div>
