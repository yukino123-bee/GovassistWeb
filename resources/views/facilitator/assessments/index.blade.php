            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($assessments as $assess)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-3.5 font-semibold text-slate-800">{{ $assess->user->name ?? 'Citizen' }}</td>
                        <td class="px-6 py-3.5">{{ $assess->service->name_en ?? 'Service' }}</td>
                        ...
                        <td class="px-6 py-3.5 text-right space-x-2">
                            <a href="{{ route('facilitator.assessments.show', $assess) }}" class="text-[10px] font-extrabold uppercase tracking-widest text-slate-500 hover:text-red-700 transition-colors">View</a>
